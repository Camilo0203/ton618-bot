...
function msgHTML(msg) {
  ...
  const timeFormatted = formatDateTime(msg.createdAt, { includeSeconds: true, literal: "a las" });
    ...
    let embedContent = `
      <div class="embed" style="border-color: ${color}">
        ...
        ${e.footer ? `<div class="embed-footer">
          ...
          ${e.timestamp ? `<span class="embed-footer-separator">•</span><span class="embed-footer-timestamp">${formatDate(e.timestamp)}</span>` : ''}
        </div>` : ''}
      </div>
    `;
